...
## 3. Non-Goals
Referrer personalization, geo, returning visitor (Y-01). Punishing long docs. Requiring Google-classic STTF as proven for all chat products (unknown). Y9 SPA `history.back` interaction (deferred). Do not use unverified 75%/94%/38% marketing stats (M-003) or breadcrumb 77%/6%/82% (Y-013).
