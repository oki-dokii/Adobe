...
## 13. Metrics
Do **not** blend into one number (K-020).

- `score(q) ∈ {1.0 answered, 0.5 partial, 0.0 unanswerable}`
- **CoreAnswerabilityRate** = stakes-weighted mean over applicable questions **excluding K9/K10/K21/K22**
- **AnswerConcentration** = mean distinct supporting pages per answered question (HotpotQA; not a five-page cutoff)
- **FreshnessAdjustedFlags** = K24 items Topic I marks stale
- W alignment: **2–3** calibration queries only
- Composite citability (AH25) is a cheap supporting metric, not live-citation proof

Weights are uncalibrated (K-030) — report per-question outcomes as the skill’s primary output.
