...
const SKILL_MICRO_SPECS: Record<SkillId, MicroDef[]> = {
  'crawl-access-audit': [
    { id: 'robots-txt', label: 'robots.txt directives', kind: 'check', dist: 38, angleShift: -0.34, level: 2 },
    { id: 'status-codes', label: 'HTTP status codes', kind: 'check', dist: 48, angleShift: -0.10, level: 2 },
    { id: 'status-403', label: '403 challenge boundary', kind: 'evidence', dist: 30, angleShift: -0.16, level: 3, parentCheckId: 'status-codes' },
    { id: 'challenges', label: 'Bot challenge responses', kind: 'check', dist: 44, angleShift: 0.16, level: 2 },
    { id: 'sitemap-xml', label: 'Sitemap indexing', kind: 'check', dist: 54, angleShift: 0.36, level: 2 },
    { id: 'sampled-pages', label: '18 sampled pages', kind: 'page', dist: 28, angleShift: 0.24, level: 3, parentCheckId: 'sitemap-xml' },
  ],
  'render-extract-audit': [
    { id: 'server-html', label: 'Server HTML shell', kind: 'signal', dist: 44, angleShift: -0.30, level: 2 },
    { id: 'client-dom', label: 'Hydrated client DOM', kind: 'signal', dist: 50, angleShift: 0.26, level: 2 },
    { id: 'main-ratio', label: 'Main text ratio (11%)', kind: 'evidence', dist: 34, angleShift: 0.04, level: 3, parentCheckId: 'client-dom' },
    { id: 'dom-stability', label: 'Layout shift & stability', kind: 'check', dist: 60, angleShift: 0.08, level: 2 },
  ],
  'site-type-classifier': [
    { id: 'template-detect', label: 'Template structure', kind: 'check', dist: 40, angleShift: -0.32, level: 2 },
    { id: 'content-taxonomy', label: 'Content taxonomy', kind: 'check', dist: 50, angleShift: -0.06, level: 2 },
    { id: 'site-class', label: 'SaaS / Web App classification', kind: 'fact', dist: 28, angleShift: 0.04, level: 3, parentCheckId: 'content-taxonomy' },
    { id: 'commerce-signals', label: 'Commerce & auth signals', kind: 'check', dist: 44, angleShift: 0.26, level: 2 },
  ],
  'entity-identity-audit': [
    { id: 'org-schema', label: 'Organization schema markup', kind: 'check', dist: 46, angleShift: -0.30, level: 2 },
    { id: 'brand-aliases', label: 'Brand alias resolution', kind: 'check', dist: 56, angleShift: 0.02, level: 2 },
    { id: 'disambiguation', label: 'Disambiguation confidence (98%)', kind: 'evidence', dist: 30, angleShift: 0.12, level: 3, parentCheckId: 'brand-aliases' },
    { id: 'kg-entity', label: 'Knowledge Graph alignment', kind: 'fact', dist: 46, angleShift: 0.32, level: 2 },
  ],
  'ai-answerability-audit': [
    { id: 'question-prompt', label: 'Synthetic query prompt', kind: 'check', dist: 44, angleShift: -0.28, level: 2 },
    { id: 'fact-retrieval', label: 'Grounding fact retrieval', kind: 'fact', dist: 56, angleShift: 0.02, level: 2 },
    { id: 'confidence-margin', label: 'Extraction confidence gap', kind: 'evidence', dist: 38, angleShift: 0.28, level: 2 },
    { id: 'substitution-risk', label: 'Source substitution vector', kind: 'evidence', dist: 30, angleShift: 0.16, level: 3, parentCheckId: 'confidence-margin' },
  ],
  'citation-extractability-audit': [
    { id: 'attributable-claims', label: 'Attributable claims', kind: 'check', dist: 46, angleShift: -0.32, level: 2 },
    { id: 'stable-anchors', label: 'Stable URI anchors', kind: 'anchor', dist: 58, angleShift: -0.06, level: 2 },
    { id: 'pricing-quote', label: 'Self-contained pricing quote', kind: 'evidence', dist: 32, angleShift: -0.16, level: 3, parentCheckId: 'stable-anchors' },
    { id: 'structured-facts', label: 'Machine-quotable syntax', kind: 'check', dist: 46, angleShift: 0.26, level: 2 },
  ],
  'freshness-audit': [
    { id: 'published-date', label: 'Published timestamp', kind: 'signal', dist: 40, angleShift: -0.24, level: 2 },
    { id: 'modified-signal', label: 'Modified header delta', kind: 'signal', dist: 54, angleShift: 0.02, level: 2 },
    { id: 'temporal-decay', label: 'Temporal decay half-life', kind: 'evidence', dist: 32, angleShift: -0.10, level: 3, parentCheckId: 'modified-signal' },
    { id: 'update-cadence', label: 'Changelog update cadence', kind: 'check', dist: 68, angleShift: 0.24, level: 2 },
  ],
  'corroboration-consistency-audit': [
    { id: 'onsite-claims', label: 'On-site statements', kind: 'signal', dist: 44, angleShift: -0.32, level: 2 },
    { id: 'external-sources', label: 'Third-party agreement sources', kind: 'signal', dist: 44, angleShift: 0.30, level: 2 },
    { id: 'consensus-fact', label: 'Contradiction check', kind: 'fact', dist: 62, angleShift: -0.02, level: 2 },
    { id: 'contradiction-count', label: '0 contradictions detected', kind: 'evidence', dist: 32, angleShift: 0.12, level: 3, parentCheckId: 'consensus-fact' },
  ],
  'engagement-handoff-audit': [
    { id: 'action-legibility', label: 'Action legibility for assistants', kind: 'check', dist: 46, angleShift: -0.24, level: 2 },
    { id: 'deep-link-schema', label: 'Direct intent deep links', kind: 'anchor', dist: 42, angleShift: -0.34, level: 2 },
    { id: 'handoff-paths', label: 'Handoff routing path', kind: 'check', dist: 58, angleShift: 0.08, level: 2 },
    { id: 'conversion-target', label: 'Conversion target endpoint', kind: 'signal', dist: 36, angleShift: 0.28, level: 3, parentCheckId: 'handoff-paths' },
  ],
  'audit-orchestrator': [],
}
