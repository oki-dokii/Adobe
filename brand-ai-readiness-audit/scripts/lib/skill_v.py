...
YMYL_TERMS = ("medical", "health", "clinic", "diagnos", "therapy", "invest", "bank", "loan", "legal advice", "attorney")
SAAS_TERMS = ("saas", "subscribe", "workspace", "platform", "api", "cloud", "login")
...
def run(snapshot: CrawlSnapshot) -> SkillResult:
    ...
    if any(t in blob for t in YMYL_TERMS) or ".gov" in host:
        votes["A"] += 3
    ...
    if any(t in blob for t in DOCS_TERMS):
        votes["D"] += 3
    if any(t in blob for t in NEWS_TERMS) or "/blog" in blob:
        votes["E"] += 2
    ...
    if ymyl and primary == "A":
        if not any(w in blob for w in ("reviewed by", "medical reviewer", "licensed", "npi ")):
            f = make_finding(
