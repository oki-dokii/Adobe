...
_CONSIDERATION = {"js_fact_lock", "d41_hidden", "pdf_only_fact", "image_locked_fact", "date_divergence", "linked_contradiction", "comparison_self_win", "flagship_gap", "expected_gap", "uncorroborated", "product_brand_gap", "schema_only_fact"}
...
_QUESTIONS = {
    ...
    "K7": ("What makes this different or unique?", "consideration", "Medium"),
...
_FINDING_INTERPRETATIONS: dict[str, str] = {
    ...
    "ymyl_no_disclaimer": "Domain operates in a sensitive commercial or professional sector but lacks standard transparent disclaimers and organizational disclosures.",
    "product_brand_gap": "Product page presents product specifications without explicit parent brand name or relationship phrasing ('by [Brand]', 'made by').",
    "schema_only_fact": "Schema.org structured data asserts material facts (such as pricing or physical address) that do not appear anywhere in human-visible page content.",
...
_FINDING_WHY_AI_CARES: dict[str, str] = {
    ...
    "ymy_disclosure": "Search quality standards enforce strict transparency requirements for sensitive health, legal, and financial topics.",
    "ymyl_no_disclaimer": "Search quality standards enforce strict transparency requirements for sensitive health, legal, and financial topics.",
    "product_brand_gap": "Research F17/F18: Entity resolution models match user queries against brand graphs; unbranded product pages cause entity-collision or orphan product attribution.",
    "schema_only_fact": "Research B-F3: Search engine guidelines and LLM verification models penalize schema-only facts as deceptive markup, discounting structured claims absent from rendered text.",
...
_FINDING_POTENTIAL_CONSEQUENCES: dict[str, str] = {
    ...
    "ymyl_no_disclaimer": "Automated systems apply severe trust penalties, excluding the site from safety-sensitive answers.",
    "product_brand_gap": "AI systems answering queries about this product fail to attribute it to the brand, or recommend generic alternatives instead.",
    "schema_only_fact": "Assistants distrust the structured data payload or omit the uncorroborated specification from direct answer cards.",
...
_FINDING_CONSEQUENCE_CHAINS: dict[str, list[str]] = {
    ...
    "product_brand_gap": [
        "Product landing page presents product specifications without explicit parent brand name or relationship phrasing ('by [Brand]', 'made by').",
        "Entity extraction and knowledge graph linking cannot unambiguously bind the product entity to the organization's Knowledge Graph node.",
        "Retrieval assistants answering product queries may fail to associate the offering with the verified brand authority.",
    ],
    "schema_only_fact": [
        "JSON-LD structured data asserts critical specifications (pricing or address) that are absent from visible HTML copy.",
        "Anti-spam and multi-modal alignment heuristics detect the discrepancy between structured markup and visible text.",
        "Retrieval pipelines discount or exclude the unverified structured attributes from answer synthesis.",
    ],
    "coverage_statement": [
...
def _exposure(f: Finding, sampled: int) -> str:
    ...
    if question in {"K6", "K13"} or f.finding_type in _DECISION or f.finding_type in {"ymyl_no_disclaimer", "ymy_disclosure"}:
        priority = "high"
    elif question in {"K4", "K5", "K7"} or f.finding_type in _CONSIDERATION:
        priority = "medium"
    else:
        priority = "low"
...
def annotate(findings: list[Finding], sampled_pages: int, per_question: dict | None = None) -> dict:
    ...
    return {"findings": enriched, "dimension_scores": dimension_scores, "overall_index": overall_index,
            ...
            "scoring_methodology": "Baseline 90 per dimension (range 15–96). Deductions per confirmed finding: Critical (-14), High (-9), Medium (-5), Low (-2). Overall index is arithmetic mean of the 4 dimensions."}
