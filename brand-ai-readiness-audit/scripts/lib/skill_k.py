...
# K9/K10/K21/K22 are expected gaps, not defects.
K3_OFFERING = re.compile(
    r"\bwe are (?:a|an)\s+(?!service\b)([a-z][a-z0-9-]{2,}(?:\s+[a-z0-9-]{2,}){0,8})"
    r"|\bwe (?:provide|offer|build|make|sell)\s+(?!service\b)([a-z][a-z0-9-]{2,}(?:\s+[a-z0-9-]{2,}){0,8})"
...
QUESTIONS = [
    ...
    {"id": "K4", "q": "Who is the intended audience?", "pats": [r"\bfor (teams|developers|enterprises|small businesses|clinicians)\b"], "home_pref": False},
...
def run(snapshot: CrawlSnapshot, question_ids: list[str] | None = None) -> SkillResult:
    ...
    for spec in QUESTIONS:
        ...
        if spec.get("expected_gap"):
            ...
            f.admission = {"emitted": "suppressed", "rule": "K9/K10"}
            findings.append(f)
            continue
