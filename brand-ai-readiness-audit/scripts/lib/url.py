...
def is_locale_path_segment(seg: str) -> bool:
    """BCP-47 or ISO 3166-1 alpha-2 first path segment (en, en-au, in)."""
    return bool(seg) and bool(re.fullmatch(r"[a-z]{2}(?:-[a-z]{2})?", seg, re.I))


def locale_stripped_path(url: str) -> str:
    parts = [p for p in (urlparse(url).path or "/").split("/") if p]
    if parts and is_locale_path_segment(parts[0]):
        parts = parts[1:]
    return "/" + "/".join(parts)
