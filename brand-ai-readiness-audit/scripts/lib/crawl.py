...
def crawl(
    ...
    smap_lastmod = {canonical_key(x["url"]): x.get("lastmod") for x in sitemap_urls if x.get("lastmod")}
    lastmods = [x.get("lastmod") for x in sitemap_urls if x.get("lastmod")]
    for p in pages:
        if p.dates is None:
            p.dates = {}
        p.dates["sitemap_lastmod"] = smap_lastmod.get(canonical_key(p.final_url or p.url))
